   ...
   var netStrs ='<div id="nets" class="netw-mack">'+
    ...
    window.onload = function(){
        var hrefs = window.location.href,
            ...
            payFs = true,
            apiUrl = htps+'wx.peilian.com',
            htpUrl = htps+'vippl.peilian.com',
            loginUrl = htps+'wx.peilian.com/site/login?returnUrl=',
            $http = axios;
            ...
            methods: {
                   ...
                   .then(function(res) {
                       if(res.data.code == 200){
                           ...
                           if(res.data.data.state >=30){
                               ...
                               window.location.href ='/info-result?t=camera'
                           }else{
                               sessionStorage.setItem('my_token',_sthis.payMiyao);
                               window.location.href ='/info-address?t=camera'
                           }
                       }else{
               ...
               goInvoiceFun(){//申请发票
                   ...
                   if(gThis.invoiceItems.invoice == null){
                       ...
                       window.location.href ='/info-invoice?t=invoice'
                        ...
                        if(gThis.invoiceItems.process_id&&gThis.invoiceItems.state){
                            if(gThis.invoiceItems.state<30){
                               ...
                               //已经开发票
                               sessionStorage.setItem('my_token',this.payMiyao);
                               window.location.href ='/info-result?t=invoice'
                           }
                       }else{
                           //是电子发票  未开发票
                           if(gThis.invoiceItems.invoice.status<0){
                               ...
                               if(gThis.invoiceItems.invoice.is_electronic_invoice == 1){
                                   window.location.href ='/info-view?t=invoice'
                               }else{
                                   window.location.href ='/info-result?t=invoice'
                               }
                           }
