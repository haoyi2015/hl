...
export default {
    ...
    methods:{
        ...
        onTimeupdate(res) {
            ...
            let sliderTimes = this.currentTime / this.currentTimeMaxTime * 100
        ...
        clickPlay(event,index,media){
            ...
            self.cur_music = index;
            self.$nextTick(
                ()=>{
                    /*
                    * 记录当前播放的音频(是否是同一个)
                     */
                    if(self.dqIndex === self.cur_music){
                        self.playFlag()
                        return
                    }else{
                        self.$refs.audio.src = media.Media
                        self.dqIndex = index //记录在播放的音频文件是那个
                        self.playing = false
                        self.playFlag()
                    }
                }
