...
if(systemFun()){
    ...
    var payFs = true,
        apiUrl = 'http://wx.peilian.com',
        htpUrl = 'http://vippl.peilian.com',
        loginUrl = 'http://wx.peilian.com/site/login?returnUrl=',
        $http = axios;
    ...
    var appPay = new Vue({
        ...
        methods: {
           ...
           timeDown () {
               var _this = this;
               //ios 时间格式只是支持2018／04／01／
               // var time = _this.endTime;
               // time=time.replace(/-/g,':').replace(' ',':');
               // time=time.split(':');
               //var endTime = new Date(time[0],(time[1]-1),time[2],time[3],time[4],time[5]);
               var endTime = new Date(_this.endTime)
               var nowTime = new Date();

               var leftTime = _this.endTime||0;//parseInt((endTime.getTime()-nowTime.getTime())/1000);
            ...
            orderDetail: function() {
                ...
                .then(function(res) {
                    if(res.data.code == 200){
                        ...
                        //全部支付 支付状态为 0
                        if(_this.detailData.pay_status==0){
                            //获取时间
