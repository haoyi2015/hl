...
<template>
    <div class="slider-swiper-container" @touchmove="fn">
        <div class="slider-default-swiper-box" :class="box" @transitionend="transitionend" @touchstart="s" @touchmove="m" @touchend="e">
			<slot/>
        </div>

        <slot name="pagination">
        	<!-- 默认提供了一个 pagination -->
			<div class="slider-pagination" v-if="pagination">
				<div class="slider-dot" v-for="(value, key) in reallySlidesNumber" :key="key" :class="{'slider-dot-active': currentSlide== key}">
				</div>
			</div>
        </slot>
        <!-- 这两个就不默认提供了 -->
...
<script>
	...
	export default {
		...
		mounted () {
			this.box = 'slider-swiper-box-' + Math.random().toFixed(2)*1000;
			setTimeout(()=>{
			...
			setDefaultSlide () {
				...
				*/
				...
				if (this.defaultSlide < 0 || this.defaultSlide > this.slidesNumber - 2 - 1) {
					throw new Error('[slider-swiper:Error]: You have set a wrong defaultSlide value with defaultSlide = ' + this.defaultSlide);
				}
