<template>
    <div class="mk-popup">
      ...
      <transition
        ...
        <div v-show="open" :class="innerClasses" @click="handleClose2" direction="column">
          <flexbox class="mk-popup-nav" v-if="title && !showClose" align="center">
            <button type="button" @click="close">返回</button>
            <flexbox-item class="mk-popup-nav-title">{{title}}</flexbox-item>
          </flexbox>
          <slot :open="open"></slot>
        </div>
      </transition>
...
<style media="screen" lang="less" scoped>
    ...
    .mk-popup-close:after,
    .mk-popup-close:before {
        content: '';
        position: absolute;
        background: #999;
        left: 50%;
...
export default {
  ...
  computed: {
    innerClasses () {
      let array = ['mk-popup-inner', 'mk-popup-' + this.direction, this.full ? 'mk-full' : '']
      if (this.direction === 'center') {
        array.push('mk-flexbox mk-flexbox-align-center mk-flexbox-content-center')
      } else {
        array.push('mk-flexbox mk-flexbox-column')
      }
      return array
    }
  },
  ...
  methods: {
    ...
    handleClose2 (e) {
      if (this.fastClose && e.target === this.$el.querySelector('.mk-popup-inner')) {
        this.close()
      }
    },
    handleBeforeLeave () {
