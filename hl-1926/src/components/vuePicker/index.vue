        ...
        <div class="picker-wheel-wrapper" ref="wheelWrapper">
          <div class="picker-wheel" v-for="(wheel, index) in pickerData" :key="index">
            <ul class="wheel-scroll">
              <li class="wheel-item" v-for="(item, index) in wheel" :key="index">{{item.name}}</li>
            </ul>
          </div>
        </div>
      </div>
...
<script>
import BScroll from './lib/bscroll.esm'
...
export default {
  ...
  methods: {
    ...
    _anchorGetter () {
      ...
      anchor = anchor.map((item, i) => {
        ...
        if (item.index) {
          ...
          index = this.pickerData && this.pickerData[i] && this.pickerData[i].indexOf(item.name) > -1
            ? this.pickerData[i].indexOf(item.name) : 0
        } else {
          index = item
        }
        return index
    ...
    _getCurrentValue () {
      const value = [],itemDate = this.pickerData
      this.wheels.forEach((wheel, i) => {
        const j = wheel.getSelectedIndex()
        value.push({
          index: itemDate[i][j]?j:'',
          name: itemDate[i][j]?itemDate[i][j].name:'',
          id: itemDate[i][j]?itemDate[i][j].id:''
        })
      })
      return value
    },
