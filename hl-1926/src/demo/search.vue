...
<div>
    ...
    <Scroller id="scroll" ref="scroll" :dataList="filmList" :pullDownRefresh="DOWN_CONFIG" :pullUpLoad="UP_CONFIG" @onPullUp="pullUpHandle" @onPullDown="pullDownHandle">
        <ul>
            <router-link class="film-list" v-for="(v,i) in filmList" tag="li" :to='{path:"/film-detail/"+v.id}'>
                <div class="film-list__img">
                    <img src="@/assets/head.jpg" alt="" />
                </div>
                <div class="film-list__detail">
                    <p class="film-list__detail__title">{{v.title}}</p>
                    <p class="film-list__detail__director">导演：{{v.directors}}</p>
                    <p class="film-list__detail__year">年份：{{v.year}}<span>{{v.stock}}</span></p>
                    <p class="film-list__detail__type">类别：{{v.genres}}<span></span></p>
                    <p class="film-list__detail__rank">评分：<span>{{v.average}}分</span></p>
                </div>
            </router-link>
        </ul>
    </Scroller>
...
<script>
import Scroller from '@/componentss/scroller.vue'
import Loading from '@/componentss/loading.vue'
...
export default {
    ...
    methods: {
        pullUpHandle(val) {
            ...
            const _this = this
            //_this.scrollElement.PullingUpWord="加载中...";
            setTimeout(() => {
                _this.scrollElement.finish("PullUp");
                _this.filmList = _this.filmList.concat(ar);
            }, 1000)
            return
            this.fetchData((res) => {
        ...
        pullDownHandle(val) {
            ...
            this.scrollElement.PullingUpWord = "刷新中...";
            setTimeout(() => {
