<template>
  <div class="player">sdadadada
    <transition name="normal" >
      <div class="normal-player" @touchstart.once="firstPlay">
        <div class="background">
          <!-- <transition name="filterR">
          <div class="filterR" v-show="currentShow === 'lyric'"></div>
          </transition> -->
          <div class="filter"></div>
          <img :src="playlists.image" width="100%" height="100%">
        </div>
        <div class="top">
          <div class="back" @click="back">
            <i class="fa fa-angle-down"></i>
          </div>
          <h1 class="title" v-html="playlists.name"></h1>
          <h2 class="subtitle" v-html="playlists.singer"></h2>
        </div>
            ...
            <div class="middle-l" v-show="currentShow === 'cd'">
              <div class="cd-wrapper">
                <div class="cd" :class="cdCls" >
                  <img :src="playlists.image" class="image">
                </div>
              </div>
            </div>
          </transition>
        ...
        <div class="bottom">
          ...
          <div class="operators">
            ...
            <div class="icon i-right">
              <i class="iconfont"  @click="toggleFavorite(playlists)" :class="getFavoriteIcon(playlists)"></i>
            </div>
          </div>
        </div>
      </div>
    ...
    <transition name="mini">
      <div class="mini-player" v-show="!fullScreen" @click.stop="open">
        <div class="icon">
          <img :class="cdCls"  :src="playlists.image" width="40" height="40">
        </div>
        <!-- <div class="text">
...
<script>
import ProgressCircle from '@/componentss/vueMusic/progress-circle'
import ProgressBar from '@/componentss/vueMusic/progress-bar'
// import Lyric from 'lyric-parser'
import Scroll from '@/componentss/vueMusic/scroll'
import Playlist from '@/componentss/vueMusic/playlist'
...
export default {
  ...
  data () {
    return {
      ...
      playingLyric: '',
      noLyric: false,
      playlists: ''//this.payList
    }
  },
  created () {
    let payList = JSON.parse(sessionStorage.getItem('payData'))
    this.playlists = payList.list[0]
    console.info(this.playlists)
    this.move = false
  },
  computed: {
    iconMode () {
  ...
  //   },
  //   url (newUrl) {
  //     this._getLyric(this.playlists.id)
  ...
  methods: {
    ...
    _resetCurrentIndex (list) {
      let index = list.findIndex((item) => {
        return item.id === this.playlists.id
      })
    ...
    ready () {
      this.songReady = true
      this.savePlayHistory(this.playlists)
    },
    next () {
      if (!this.songReady) {
        return
      ...
      let music = [
        {
            "title":"依恋",
            "author":"蔡淳佳",
            "pic":"https://api.hibai.cn/music/Music/Music?id=210089&type=pic",
            "url":"https://api.hibai.cn/music/Music/Music?id=210089&type=url",
            "lrc":"https://api.hibai.cn/music/Music/Music?id=210089&type=lrc"
        },
        {
            "title":"一见钟情",
            "author":"蓝心湄",
            "pic":"https://api.hibai.cn/music/Music/Music?id=259714&type=pic",
            "url":"https://api.hibai.cn/music/Music/Music?id=259714&type=url",
            "lrc":"https://api.hibai.cn/music/Music/Music?id=259714&type=lrc"
        },
        {
            "title":"一生守候",
            "author":"王若琳",
            "pic":"https://api.hibai.cn/music/Music/Music?id=25698049&type=pic",
            "url":"https://api.hibai.cn/music/Music/Music?id=25698049&type=url",
            "lrc":"https://api.hibai.cn/music/Music/Music?id=25698049&type=lrc"
        }
      ]
      this.url = music[0].url
      return
      getSong(id).then((res) => {
